...
from backend.pulse_brain.llm_interface import generate_response
from backend.pulse_helpers.postgre_helpers import create_active_session_for_user, get_active_session_for_user
...
async def process_and_reply(sender_id: str, phone_number: str, message_body: str, response_text: str):
    ...
    req = UserRequest(phone_number=phone_number, message=message_body)
    user = validate_user(req)

    active_session = get_active_session_for_user(user.user_id)
    print(f"Active session for user {user.user_id}: {active_session}")
    if not active_session:
        active_session = create_active_session_for_user(user.user_id)
    else:
...
async def openwa_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    if event == "message.received" and not data.get("isGroupMsg", False):
        ...
        if sender_id and message_body:
            ...
            history = get_active_session_for_user(phone_number)
            if history:
                response_text = await generate_response(message_body, history, None)
            else:
                response_text = await generate_response(message_body, [], None)
            background_tasks.add_task(process_and_reply, sender_id, phone_number, message_body, response_text)
